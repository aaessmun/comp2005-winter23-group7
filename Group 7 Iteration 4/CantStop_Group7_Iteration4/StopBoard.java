...
public class StopBoard extends JFrame implements MouseListener{
    ...
    public JPanel dicePanel, leftDicePanel, rightDicePanel;
    ...
    // arraylists
    public ArrayList<StopColumn> boardColumns;
    private ArrayList<Integer> rollValues;
    private ArrayList<DiceCombo> combinations;
    public ArrayList<Integer> placedRunners;
    ...
    private HashMap<Player, Comp> compMap;
    private HashMap<Player, ArrayList<Integer>> playerColumns;
    ...
    public StopBoard(int numPlayers, HashMap<Player, ArrayList<String>> playerDetail, String getLoadGame){
		...
		setResizable(true);
		setVisible(true);
        if(getLoadGame.contains("player")){
            loadGameData(getLoadGame);
        }
        checkForComp();
    }
    ...
    public void setComps(){
        String type;
        for(Player p :playerDetailMap.keySet()){
            type = playerDetailMap.get(p).get(3);
            if(type.equals("Easy") ){
                compMap.put(p, new EasyComp(this));
            }
            else if(type.equals("Hard")){
                compMap.put(p, new HardComp(this));
            }
    ...
    public void setRunnerDisplay(int amt){
        ...
        runnerPanel.add(new JLabel(new ImageIcon(getClass().getResource("assets/" + colourMap.get(currentPlayer).toLowerCase()+  "piece.png"))));
        for(int i = 0; i < amt; i++){
            runnerPanel.add(new JLabel(new ImageIcon(getClass().getResource("assets/runner.png"))));
        }
    }
    /** Generate Dice
     */
    ...
    public void columnSelected(StopColumn selected){
        if(selected.isActive()){
            ...
            if(selected.columnWon()){
                playerColumns.get(currentPlayer).add(boardColumns.indexOf(selected));
                if(playerColumns.get(currentPlayer).size() == 3){
    ...
    public void gameOver(Player winner){
        String winText;
        for(StopColumn col : boardColumns){
            for(StopTile t : col.column){
                t.clear();
                t.setBackground(Color.yellow);
            }
        }
        winText = "Congratulations, " + nameMap.get(winner)
        +" \nYou are victorious!\n You played hard, and did not stop! \n"
        + "We now suggest that you take a little break\n"
        + "We will see you next time!!!";
        JOptionPane.showMessageDialog(this, winText);
    ...
    public void passTurn(boolean Bust){
        ...
        if(!comp) JOptionPane.showMessageDialog(this, text);
    ...
    public void checkForComp(){
        ...
        Object c = compMap.get(currentPlayer);
    ...
    public ArrayList<Integer> getActiveColumns(){
        ArrayList<Integer> active = new ArrayList<>();
        for(StopColumn col : boardColumns){
            if(col.isActive()){
                active.add(boardColumns.indexOf(col) + 2);
            }
        }
        return active;
    }

    public ArrayList<DiceCombo> getCombinations(){
    ...
    */
    ...
    public String getTurnStatus(){
        return turnStatusLabel.getText();
    }
    ...
    public String passData(){
        ...
        for(int i = 0; i < NUM_PLAYERS; i++){
            ...
            if(compMap.get(player) instanceof EasyComp) data += "Easy\n";
            else if(compMap.get(player) instanceof HardComp) data += "Hard\n";
            else data += "Human\n";
    ...
    public void loadGameData(String data){
        ...
        this.NUM_PLAYERS = (loadLines.length - 1) / 5;
        colourMap.clear();
        nameMap.clear();
        playerPieces.clear();
        compMap.clear();

        for(int i = 0; i < NUM_PLAYERS * 5; i+=5){
            loadPlayer = Player.valueOf(loadLines[i]);
            colourMap.put(loadPlayer, loadLines[i + 1]);
            nameMap.put(loadPlayer, loadLines[i + 2]);
            String comp = loadLines[i + 3];
            if(comp.equals("Easy")) compMap.put(loadPlayer, new EasyComp(this));
            else if (comp.equals("Hard")) compMap.put(loadPlayer, new HardComp(this));
            pieces = new ArrayList<>();
            for(String s : loadLines[i + 4].split(",")){
    ...
    public void initColumnsWon(){
        playerColumns = new HashMap<>();
        Player[] players = {Player.player1, Player.player2, Player.player3, Player.player4};
        for(int i = 0 ; i < NUM_PLAYERS; i++){
            playerColumns.put(players[i], new ArrayList<Integer>());
        }
    }
